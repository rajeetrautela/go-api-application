...
func cleanupTestDB(t *testing.T) {
	err := config.DB.Exec("TRUNCATE TABLE items RESTART IDENTITY CASCADE").Error
	if err != nil {
		t.Fatalf("Failed to clean up DB: %v", err)
	}
}

func TestRepositoryCRUD(t *testing.T) {
	// Clean tables before starting test
	cleanupTestDB(t)

	// Use global config.DB and repository functions
